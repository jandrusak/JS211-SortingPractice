  ...
  const sumEvens = stringsToNumbs.filter(num => num%2==0)
  const reducedEvens = sumEvens.reduce((a, c)=> a + c)

  console.log(sumEvens);
  console.log(reducedEvens)

  // Find the index of the first value when added to it's index = 512 (#ATX!!)
    const atxIdx = "512"
    let idx =strNums.indexOf(atxIdx)        
 console.log(`index: ${atxIdx}, value: ${strNums[atxIdx]}`);
  ...
  const weatherStates = weather.map(ob => ob.weather_state_name)
  console.log(weatherStates)
  const uniqueArr = [...new Set(weatherStates)];
  console.log(uniqueArr);

  //find the id of the object in weather that has a min_temp of 15.915
  const idealTemp = weather.getElementById(min_temp)
